...
def calculate_and_visualize_rgb_indices(image_path, output_dir):
    try:
        ...
        exg = 2 * G - R - B
        # ExGR (Excess Green minus Excess Red Index): ExG - (1.4*R - G)
        exr = 1.4 * R - G
        exgr = exg - exr
        indices['ExGR'] = calculate_masked_index(exgr)
        colormaps['ExGR'] = 'RdYlGn'
...
def run_image_processor_with_gui_exgr(image_path=None):
    """
...
if __name__ == "__main__":
    # Un fișier dummy pentru demonstrație dacă nu există
    ...
    run_image_processor_with_gui_exgr(dummy_image_path)