...
def calculate_and_visualize_rgb_indices(image_path, output_dir):
    try:
        ...
        # TGI (Triangular Greenness Index): G - 0.37*R - 0.63*B
        tgi = G - 0.37 * R - 0.63 * B
        indices['TGI'] = calculate_masked_index(tgi)
        colormaps['TGI'] = 'summer'
        ...
        for name, index_array in indices.items():
            ...
            if np.all(np.isnan(index_array[non_black_mask])) and np.any(non_black_mask):
                print(
                    f"Se omite '{name}' deoarece nu conține date valide pentru pixelii non-negri (potențială împărțire la zero sau toate valorile sunt NaN).")
                continue

            # Dacă întreaga imagine (inclusiv fundalul negru) este NaN
            if np.all(np.isnan(index_array)):
                print(
                    f"Se omite '{name}' deoarece întregul array al indicelui este NaN (imaginea ar putea fi complet neagră sau probleme la calcul).")
                continue
...
def run_image_processor_with_gui_tgi(image_path=None):
    """
